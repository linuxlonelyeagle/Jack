...
void Parser::advance()
{
    ...
    if (current_cmd.find("push") != -1 || current_cmd.find("pop")!= -1 ||
        current_cmd.find("function") != -1 || current_cmd.find("call") != -1) {
        ...
        if (token1 == "push") {
            ...
            command.type = Parser::Command::C_FUNCTION;
            command.cmd = token1;
            command.arg1 = token2;
            command.arg2 = std::stoi(token3.c_str());
        }   else if (token1 == "call")  {
            command.type = Parser::Command::C_CALL;
            command.cmd = token1;
            command.arg1 = token2;
            command.arg2 = std::stoi(token3.c_str());
        }    
    } else if (current_cmd.find("label") != -1 || 
    current_cmd.find("goto") != -1 || 
    current_cmd.find("if-goto") != -1) {
        auto index1 = current_cmd.find_first_of(" ");
        auto index2 = current_cmd.find(" ", index1+1);
        ...
        token2 = removespace(token2);
/*
        std::cout << "token1:" << token1 << " " << "token2:" << token2 << std::endl;
        std::cout << token1.size() << " " << token2.size() << std::endl;

*/
        if (token1 == "label") {
            command.type = Parser::Command::C_LABLE;
        } else if (token1 == "goto") {
            command.type = Parser::Command::C_GOTO;
        } else if (token1 == "if-goto") {
            command.type = Parser::Command::C_IF;
        }
        command.cmd = token1;
        command.arg1 = token2;
    } else {
        if (current_cmd == "return")
            command.type = Parser::Command::C_RETURN;
        else 
        command.type = Parser::Command::C_ARITHMETIC;    
        token1 = current_cmd;
        token1 = removespace(token1);
        command.cmd = token1; 
        /*
